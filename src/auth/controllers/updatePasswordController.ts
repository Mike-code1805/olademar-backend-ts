...
export const updatePasswordController = async (req: Request<{ id: string; token: string }, {}, { password: string }>, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    ...
    const userValid = await updateOneUserService(id, { password: newPassword });
